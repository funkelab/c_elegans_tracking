...
def center_pad(arrs, shape):
    ...
    for i, arr in enumerate(arrs):
        ...
        pad_width = tuple(
            (half_pad_amt[dim], pad_amt[dim] - half_pad_amt[dim]) for dim in range(ndim)
        )
...
def pad_to_seam_cell(arrs, seam_cell_locs):
    ...
    print(f"{max_loc=}")
    offsets = [
        tuple(int(max_loc[dim] - seam_cell_loc[dim]) for dim in range(ndim))
        for seam_cell_loc in seam_cell_locs
    ]
    print(f"{offsets=}")
    shapes = []
    ...
    for arr, offset in zip(arrs, offsets, strict=True):
        upper_pad_amt = tuple(
            max_shape[dim] - arr.shape[dim] - offset[dim] for dim in range(ndim)
        )
...
def load_straightened(time_range):
    ...
    manual_annotations_path = (
        base_path / "Annotations" / "Straightened" / "Hand_Annotated"
    )
    carsen_annotations_path = (
        base_path / "Annotations" / "Straightened" / "Carsen_Annotations"
    )
    assert manual_annotations_path.is_dir(), (
...
if __name__ == "__main__":
    ...
    carsen_points = load_points(
        carsen_annotations_path, time_range, offsets, straightened=straightened
    )
    viewer.add_points(
        data=carsen_points, name="carsen_annotations", face_color="pink", size=5
    )
    ...
    manual_graph = load_graph(
        manual_annotations_path, path_after_time, time_range, offsets
    )
    tracks_viewer = TracksViewer.get_instance(viewer)
    tracks_viewer.tracks_list.add_tracks(
        SolutionTracks(manual_graph, ndim=4), "manual_annotations"
    )

    if args.seam_cells:
        seam_cell_raw_path = base_path / "RegA" / dirname
        _test_exists(seam_cell_raw_path)
        seam_cell_raw, seam_cell_offsets = load_raw(
            seam_cell_raw_path,
            time_range,
